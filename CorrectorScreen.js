...
const CorrectorScreen = ({ onNavigate, templates, onAddReport }) => { 
  const [step, setStep] = useState('selectTemplate');
  const [selectedTemplate, setSelectedTemplate] = useState(null);
  
  // ===== NOVOS ESTADOS ADICIONADOS =====
  const [studentName, setStudentName] = useState('');
  const [studentMatricula, setStudentMatricula] = useState(''); // Estado para Matrícula
  const [studentTurma, setStudentTurma] = useState('');     // Estado para Turma
  ...
  const startCorrection = (template) => {
    setSelectedTemplate(template);
    setStep('camera');
  };

  // Função unificada para salvar o relatório (agora com matrícula e turma)
  const handleSaveReport = () => {
    if (!studentName) { // Simplificamos a validação para o protótipo
      alert('Por favor, insira pelo menos o nome do aluno.');
      return false;
    }
    // Passa todos os dados coletados para o App.js
    onAddReport(selectedTemplate, correctionResult, studentName, studentMatricula, studentTurma);
  ...
  // ... (Código do 'selectTemplate', 'camera' e 'processing' continuam idênticos)
          ...
          <View style={styles.card}>
            ...
            <View style={styles.inputContainer}>
              ...
              <TextInput 
                ...
                keyboardType="numeric" // Teclado numérico para matrícula
              />
            </View>
  ...
  // --- CÓDIGO IDÊNTICO PARA OS OUTROS PASSOS ---
        ...
        <ScrollView contentContainerStyle={styles.scrollContent}>
          {templates.length === 0 ? (
            <EmptyState
              ...
              onButtonPress={() => onNavigate('createTemplate')}
            />
          ) : (
            <>
...
export default CorrectorScreen;