...
const StatCard = ({ icon, label, value, color, fullWidth = false }) => (
  <View style={[styles.statCard, fullWidth && styles.statCardFullWidth]}>
    ...
    <View style={styles.statTextContainer}>
      {/* Ajuste para valor não ser sempre bold */}
...
const ReportsScreen = ({ onNavigate, templates }) => {
    ...
    return (
      <SafeAreaView style={styles.container}>
        ...
        <ScrollView contentContainerStyle={styles.scrollContent}>
          ...
          {/* ===== MÉTRICAS INDIVIDUAIS ATUALIZADAS ===== */}
          <View style={styles.individualStatsContainer}>
            {/* Usamos StatCard para exibir Matrícula e Turma */}
            ...
            <View style={{height: 10}}/> {/* Espaçador */}
  ...
  if (viewingTemplate) {
    // ... (Código do Nível 2 permanece idêntico) ...
    const results = viewingTemplate.results || [];
    const aggregateMetrics = { // Simulando métricas agregadas
  ...
  // --- NÍVEL 1: LISTA DE AVALIAÇÕES ---
  if (!viewingTemplate && !viewingResult) {
    // ... (Código do Nível 1 permanece idêntico) ...
     return (
      <SafeAreaView style={styles.container}>
        <StatusBar barStyle="light-content" backgroundColor="#346a74" />
        <View style={styles.headerContainer}>
        ...
        <ScrollView contentContainerStyle={styles.scrollContent}>
          {templates.length === 0 ? (
            <EmptyState
              ...
              onButtonPress={() => onNavigate('corrector')}
            />
          ) : (
            <>
